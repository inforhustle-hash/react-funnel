...
export default function App() {const params = new URLSearchParams(window.location.search);const src = params.get("src") || "direct";

const [hoverMain, setHoverMain] = useState(false);const [hoverExtra, setHoverExtra] = useState(false);const [hoverHerc, setHoverHerc] = useState(false);
...
const makeTrackUrl = (offer, url) =>https://n8n-production-01ac.up.railway.app/webhook/t +?src=${encodeURIComponent(src)} +&offer=${encodeURIComponent(offer)} +&url=${encodeURIComponent(url)};

const trackClick = (offer, url) => {try {const img = new Image();img.src = ${makeTrackUrl(offer, url)}&_ts=${Date.now()};} catch (e) {console.error("Tracking failed", e);}};

return (<ahref={ctaFinalUrl}className="logo-container"onClick={() => trackClick("main-cta", ctaFinalUrl)}>
